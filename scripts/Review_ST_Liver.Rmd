...
####Reviewer comment: 
A major power of the approach is the ability to simultaneously record the distances of each spot from both the portal vein and the central vein. This enables assessing how the expression of each gene is affected by these two distances, potentially leading to novel insights into regulation of hepatocyte zonation. Hepatocyte zonation is shaped by both morphogens secreted by the non-parenchymal cells that surround the central vein (e.g. Wnt and Rspo3) as well as blood-borne factor such as hormones, nutrients and oxygen (for a list of some of these putative targets see PMID 28166538, Supplementary Table 4). It would be interesting to perform multi-linear regression of the expression of each gene vs. both the distance to the closest portal node and to the closest central vein. One could then use Bayesian Information Criterion or other methods to assess which of these distances is the stronger explanatory variable. I would expect classic Wnt target pericentral genes such as Axin2, Lgr5 and Cyp2e1, to be strongly affected by the distance to the central vein, classic glucagon targets should be periportal (PMID 29555772), whereas for some genes such as Cyp2f2, which is both a Wnt-inhibited gene and a RAS-activated gene (periportal signal) identifying the dominant distance could be very interesting.

We used the sugested genes for wnt and glucagon targets 

````{r, }

#get DEG between cluster 1 and 2 
m.veins <- FindMarkers(se.cca, ident.1 = 1, ident.2 = 2)

# + values represent portal genes, - values represent central genes
m.veins <- m.veins[order(-m.veins$avg_log2FC),]
...
cht.m <- subset(m.veins, rownames(m.veins) %in% CHT)
nrow(cht.m)

write.table(rownames(cht.m[c(1:2,20:21),]), "~/Desktop/PhDprojectFranziskaHildebrandt/ST-liver/uninfectedLiver-sequenced/ST-mLiver/data/gene-lists/ans_1/hypoxia_short.txt", quote = F, row.names = F, col.names = F)
...
pht.m <- subset(m.veins, rownames(m.veins) %in% PHT)

write.table(rownames(pht.m[c(1:2,20:21),]), "~/Desktop/PhDprojectFranziskaHildebrandt/ST-liver/uninfectedLiver-sequenced/ST-mLiver/data/gene-lists/ans_1/hormones_short.txt", quote = F, row.names = F, col.names = F)
...
har.m <- subset(m.veins, rownames(m.veins) %in% HAR)
write.table(rownames(har.m[c(1:2,31:32),]), "~/Desktop/PhDprojectFranziskaHildebrandt/ST-liver/uninfectedLiver-sequenced/ST-mLiver/data/gene-lists/ans_1/ras_short.txt", quote = F, row.names = F, col.names = F)
...
n.count.Hamp <- sapply(se.sub.l, function(b){
  sum(b@assays$SCT@counts[rownames(b@assays$SCT@counts) == "Hamp"])
})

#get counts for Hamp2

n.count.Hamp2 <- sapply(se.sub.l, function(b){
  sum(b@assays$SCT@counts[rownames(b@assays$SCT@counts) == "Hamp2"])
})

#get counts for Igfbp2

n.count.Igfbp2 <- sapply(se.sub.l, function(b){
  sum(b@assays$SCT@counts[rownames(b@assays$SCT@counts) == "Igfbp2"])
})
...
p.hamp <- ggplot(df, aes(rownames(df), log10(n.count.Hamp), fill = rownames(df))) + 
...
p.hamp2 <- ggplot(df, aes(rownames(df), log10(n.count.Hamp2), fill = rownames(df))) + 
      geom_bar(stat = "identity") + 
      scale_fill_manual(values = cluster.cols) + 
      scale_x_discrete(labels = c("0", "1", "2", "3", "4", "5"), name = "cluster") + 
...
p.igfbp2 <- ggplot(df, aes(rownames(df), log10(n.count.Igfbp2), fill = rownames(df))) + 
      geom_bar(stat = "identity") + 
      scale_fill_manual(values = cluster.cols) + 
      scale_x_discrete(labels = c("0", "1", "2", "3", "4", "5"), name = "cluster") + 
...
```{r, visulize individual genes in violin plots}
#compare to violinplot depiction: 

v.hamp <- VlnPlot(se.cca,
        features = "Hamp",
        cols = cluster.cols)
v.hamp2 <- VlnPlot(se.cca,
        features = "Hamp2",
        cols = cluster.cols)
v.igfbp2 <- VlnPlot(se.cca,
        features = "Igfbp2",
        cols = cluster.cols)

png("~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/cluster0_validation/ncount_hamp_violin.png", width = 12, height = 8, units = "cm", res = 600)
print(v.hamp)
dev.off()

png("~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/cluster0_validation/ncount_hamp2_violin.png", width = 12, height = 8, units = "cm", res = 600)
v.hamp2
dev.off()

png("~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/cluster0_validation/ncount_igfbp2_violin.png", width = 12, height = 8, units = "cm", res = 600)
v.igfbp2
dev.off()
...
markers.cca.0 <- FindMarkers(se.cca, ident.1 = 0, only.pos = T)
markers.cca.0 <- subset(markers.cca.0, p_val_adj < 0.05)

write.table(markers.cca.0, "~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/cluster0_validation/markers_cluster0.tsv", row.names = rownames(markers.cca.0), quote = F, sep = "\t")

png("~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/cluster0_validation/heatmap-0.25-lgFC.png", width = 22, height = 16, res = 600, units = "cm")
DoHeatmap(se.cca, features = rownames(markers.cca.0), size = 4, angle = 45, group.colors = cluster.cols)
dev.off()
...
cca.im <- se.cca@assays$SCT[rownames(se.cca@assays$SCT@counts) %in% go.se.cca$external_gene_name ,]
...
#order the markers in avgFC and use the top 3 highest and lowest avgFC

immune.markers <- immune.markers[order(-immune.markers$avg_log2FC),]
immune.short <- immune.markers[c(1:3, 27:29),]

write.table(as.vector(rownames(immune.short)), "~/Desktop/PhDprojectFranziskaHildebrandt/ST-liver/uninfectedLiver-sequenced/ankarliver/data/gene-lists/marker-genes/st-lec/immune_genes_short.txt", row.names = F, col.names = F, quote = F)

write.table(immune.markers, "~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/immune_LEC_zonation/immune_markers.tsv", row.names = rownames(immune.markers), quote = F, sep = "\t")
...
I extracted a gene signature for Kupffer-cell signature and other LECs from the DGEA of [PMID: 30222169](https://pubmed-ncbi-nlm-nih-gov.ezp.sub.su.se/30222169/), see: "review-ST-Liver-scAnalysis.Rmd"
...
write.table(kcell.markers, "~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/immune_LEC_zonation/kcell_markers.tsv", row.names = rownames(kcell.markers), quote = F, sep = "\t")
...
endo.short <- endo.markers[c(1:3, 26:28),]
...
write.table(as.vector(rownames(endo.markers)), "~/Desktop/PhDprojectFranziskaHildebrandt/ST-liver/uninfectedLiver-sequenced/ankarliver/data/gene-lists/marker-genes/st-lec/endo/st-endo_all.txt", row.names = F, col.names = F, quote = F)

write.table(endo.markers, "~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/immune_LEC_zonation/endo_markers.tsv", row.names = rownames(endo.markers), quote = F, sep = "\t")
...
write.table(neut.markers, "~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/immune_LEC_zonation/neut_markers.tsv", row.names = rownames(neut.markers), quote = F, sep = "\t")
...
write.table(pDC.markers, "~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/immune_LEC_zonation/pDC_markers.tsv", row.names = rownames(pDC.markers), quote = F, sep = "\t")
...
write.table(lcm.markers, "~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/immune_LEC_zonation/lcm_markers.tsv", row.names = rownames(lcm.markers), quote = F, sep = "\t")

```

```{r, HSC signature from PMID: 31722201}


#extract genes from expression matrix

m.hsc <- FindMarkers(se.fibrotic, ident.1 = "HSC", only.pos = T, logfc.threshold = 1)
m.hsc <- subset(m.hsc, p_val_adj < 0.05)
m.hsc <- subset(m.hsc, rownames(m.hsc) %in% rownames(se.cca@assays$RNA))
hsc.all <- rownames(m.hsc)

hsc.mat <- se.cca@assays$SCT[rownames(se.cca@assays$SCT) %in% hsc.all ,]
se.cca[["hsc"]] <- CreateAssayObject(counts = hsc.mat)


hsc.markers <- FindMarkers(se.cca, assay = "hsc", ident.1 = 1, ident.2 = 2, logfc.threshold = 0.001)

hsc.markers <- subset(hsc.markers, p_val_adj < 0.05)
hsc.markers <- hsc.markers[order(-hsc.markers$avg_log2FC),]
hsc.short <- hsc.markers[c(2:4, 30:32),]


write.table(as.vector(rownames(hsc.short)), "~/Desktop/PhDprojectFranziskaHildebrandt/ST-liver/uninfectedLiver-sequenced/ST-mLiver/data/gene-lists/ans_4/hsc-short.txt", row.names = F, col.names = F, quote = F)

write.table(hsc.markers, "~/Desktop/PhDprojectFranziskaHildebrandt/ST-liver/uninfectedLiver-sequenced/ST-mLiver/data/gene-lists/ans_4/hsc-all.tsv", sep = "\t", row.names = rownames(hsc.markers), col.names = T, quote = F)
...
pheatmap(phmat[rownames(endo.markers),], fontsize_row = 8, cellwidth = 15, cellheight = 15, clustering_distance_rows = "euclidean", cluster_cols = F, color = colorRampPalette(c("#440154" ,"#21908C", "#FDE725"))(1000))

##neutrophils
pheatmap(phmat[rownames(neut.markers),], fontsize_row = 8, cellwidth = 15, cellheight = 15, clustering_distance_rows = "euclidean", cluster_cols = F, color = colorRampPalette(c("#440154" ,"#21908C", "#FDE725"))(1000))

##pDCs

#create matrix for averaged expression-values between -1.5 and 1.5 (this can be adjusted)
pheatmap(phmat[rownames(pDC.markers),], fontsize_row = 8, cellwidth = 15, cellheight = 15, clustering_distance_rows = "euclidean", cluster_cols = F, color = colorRampPalette(c("#440154" ,"#21908C", "#FDE725"))(1000))

##LCMs
pheatmap(phmat[rownames(lcm.markers),], fontsize_row = 8, cellwidth = 15, cellheight = 15, clustering_distance_rows = "euclidean", cluster_cols = F, color = colorRampPalette(c("#440154" ,"#21908C", "#FDE725"))(1000))

##HSCs
pheatmap(phmat[rownames(hsc.markers),], fontsize_row = 8, cellwidth = 15, cellheight = 15, clustering_distance_rows = "euclidean", cluster_cols = F, color = colorRampPalette(c("#440154" ,"#21908C", "#FDE725"))(1000))
...
#immunity markers 
png("~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/immune_LEC_zonation/immunegenes_DEG_1_2.png", width = 10, height = 20, units = "cm", res = 300)
...
#Kupffer cells
png("~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/immune_LEC_zonation/Kupffer_DEG_1_2.png", width = 10, height = 15, units = "cm", res = 300)
...
#endothelial cell
png("~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/immune_LEC_zonation/Endo_DEG_1_2.png", width = 10, height = 15, units = "cm", res = 300)
...
#neutrophils
png("~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/immune_LEC_zonation/Neutro_DEG_1_2.png", width = 10, height = 15, units = "cm", res = 300)
...
#pDCs
png("~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/immune_LEC_zonation/pDCs_DEG_1_2.png", width = 10, height = 15, units = "cm", res = 300)
...
#LCMs
png("~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/immune_LEC_zonation/LCMs_DEG_1_2.png", width = 10, height = 15, units = "cm", res = 300)
...
##HSCs
png("~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev1/immune_LEC_zonation/HSCs_DEG_1_2.png", width = 10, height = 25, units = "cm", res = 300)
print(pheatmap(phmat[rownames(hsc.markers),], fontsize_row = 8, cellwidth = 15, cellheight = 15, clustering_distance_rows = "euclidean", cluster_cols = F, color = colorRampPalette(c("#440154" ,"#21908C", "#FDE725"))(1000)))
dev.off()
...
r.l <- c(FAMr,OXPr,GSPr, WNTr ,G_GNr,GLMr,FMMr,PPPr)

df.r <- data.frame(kegg_pathway = c("Fatty acid metabolism", "Oxidative phosphorylation", "Glucagon signaling pathway" ,"WNT signaling pathway", "Glycolysis/Gluconeogenesis", "Glycerolipid metabolism",  "Fructose and mannose metabolism", "Pentose phosphate pathway"), r.l)

df.r <- df.r[order(df.r$r.l),]
df.r$id <- ifelse(df.r$r.l < 0, paste("central"), paste("portal")) 

#plot the result
png("~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev3/kegg-pathway-marker-ratios.png", width = 20, height = 15, units = "cm", res = 600)
...
#CN73-C1
CN73_C1_CpS <- read.table("~/Box/Hildebrandt-Ankarklev-Liver/liver_cell_segmentation/results/CN73_C1/cells_per_spot_table.tsv", header = T)
...
#CN73-D1
CN73_D1_CpS <- read.table("~/Box/Hildebrandt-Ankarklev-Liver/liver_cell_segmentation/results/CN73_D1/cells_per_spot_table.tsv", header = T)
CN73_D1_CpS$spot_cca <- paste("A",CN73_D1_CpS$spot, "2", sep = "_")

#CN73-E2
CN73_E2_CpS <- read.table("~/Box/Hildebrandt-Ankarklev-Liver/liver_cell_segmentation/results/CN73_E2/cells_per_spot_table.tsv", header = T)
CN73_E2_CpS$spot_cca <- paste("A",CN73_E2_CpS$spot, "3", sep = "_")


#cells_spot_73 <- dplyr::bind_rows(CN73_C1_CpS, CN73_D1_CpS, CN73_E2_CpS)
#cells_spot_73$within <- NULL
#cells_spot_73$spot <- NULL

#CN16-D2
CN16_D2_CpS <- read.table("~/Box/Hildebrandt-Ankarklev-Liver/liver_cell_segmentation/results/CN16_D2/cells_per_spot_table.tsv", header = T)
CN16_D2_CpS$spot_cca <- paste("B",CN16_D2_CpS$spot, "2", sep = "_")

#CN16-E2
CN16_E2_CpS <- read.table("~/Box/Hildebrandt-Ankarklev-Liver/liver_cell_segmentation/results/CN16_E2/cells_per_spot_table.tsv", header = T)
CN16_E2_CpS$spot_cca <- paste("B",CN16_E2_CpS$spot, "4", sep = "_")

#cells_spot_16 <- dplyr::bind_rows(CN16_D2_CpS, CN16_E2_CpS)
#cells_spot_16$within <- NULL
#cells_spot_16$spot <- NULL

#CN65-D1
CN65_D1_CpS <- read.table("~/Box/Hildebrandt-Ankarklev-Liver/liver_cell_segmentation/results/CN65_D1/cells_per_spot_table.tsv", header = T)
CN65_D1_CpS$spot_cca <- paste("C", CN65_D1_CpS$spot, "3", sep = "_")

#CN65-D2
CN65_D2_CpS <- read.table("~/Box/Hildebrandt-Ankarklev-Liver/liver_cell_segmentation/results/CN65_D2/cells_per_spot_table.tsv", header = T)
CN65_D2_CpS$spot_cca <- paste("C", CN65_D2_CpS$spot, "4", sep = "_")

#CN65-E1
CN65_E1_CpS <- read.table("~/Box/Hildebrandt-Ankarklev-Liver/liver_cell_segmentation/results/CN65_E1/cells_per_spot_table.tsv", header = T)
CN65_E1_CpS$spot_cca <- paste("C", CN65_E1_CpS$spot, "5", sep = "_")

#cells_spot_65 <- dplyr::bind_rows(CN65_D1_CpS, CN65_D2_CpS, CN65_E1_CpS)
#cells_spot_65$within <- NULL
#cells_spot_65$spot <- NULL
...
png("~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev3/cell_count_histgram.png", width = 20, height = 12, units = "cm", res = 600)
print(hist(cca_cells_spot$count, main = "", xlab = "ncell/spot", plot = T, xlim = range(0:150), ylim = range(0:1500)))
dev.off()
...
png("~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev3/cell_count_cluster_violin_cc_norm.png",  width = 3000, height = 1500, res = 300)
print(VlnPlot(se.cca_cc,
        features = "cell_count",
        cols = cluster.cols.cc))
dev.off()

png("~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev3/cell_count_cluster_umap_cc_norm.png", width = 8400, height = 6000, res = 600)
print(FeaturePlot(se.cca_cc, features = "cell_count",
            label = T, 
            label.size = 12,
            pt.size = 2,
            cols = c("darkslategray2", "darkslategray4", "darkslategrey")) + 
...
se1 <- SubsetSTData(se.cca_cc, spots = rownames(subset(se.cca[[]], sample_id %in% c(8))))

gg <- data.frame(nCount = se.cca@meta.data$nCount_RNA,
                 nCell = se.cca@meta.data$cell_count)

png("~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev3/cellcount_cor/cell_count_cor.png", width = 4200, height = 3000, res = 600)
...
```{r, run correlation analysis plot with cell_count and unfiltered nCount of data}

#get unfiltered count data and spot coordinates for each section 

df.1 <- data.frame(spot_crd = rownames(se1[[]]), nCount_RNA = se1@meta.data$nCount_RNA)
df.2 <- data.frame(spot_crd = rownames(se2[[]]), nCount_RNA = se2@meta.data$nCount_RNA)
df.3 <- data.frame(spot_crd = rownames(se3[[]]), nCount_RNA = se3@meta.data$nCount_RNA)

head(df.1)

colnames(cells_spot_73) <- c("nCell", "spot_crd")

colnames(cells_spot_16) <- c("nCell", "spot_crd")
  
colnames(cells_spot_65) <- c("nCell", "spot_crd")

df.1 <- left_join(df.1, cells_spot_73, by = "spot_crd")
df.2 <- left_join(df.2, cells_spot_16, by = "spot_crd")
df.3 <- left_join(df.3, cells_spot_65, by = "spot_crd")

allRNA_cell <- dplyr::bind_rows(df.1, df.2, df.3)

library(ggpubr)
#png("~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev3/cell_count_RNA_count_cor.png", width = 4200, height = 3000, res = 600)
print(ggplot(allRNA_cell, aes((nCount_RNA),(nCell))) +
  geom_point() +
  stat_cor(method = "pearson"))
#dev.off()
...
FeatureOverlay(se.cca, features = "cell_count", spots = rownames(subset(se.cca[[]], cell_count == 0)), sampleids = 8)

ST.FeaturePlot(se.cca, features = "cell_count",indices = c(6:8), ncol = 3)
...
write.table(CN73_cells_spot, "~/Desktop/CN73_cells_spot.tsv", quote = F, row.names = rownames(CN73_cells_spot))
#Add the information to the meta-data
...
write.table(CN16_cells_spot, "~/Desktop/CN16_cells_spot.tsv", quote = F, row.names = rownames(CN16_cells_spot))
...
write.table(CN65_cells_spot, "~/Desktop/CN65_cells_spot.tsv", quote = F, row.names = rownames(CN65_cells_spot))
#Add the information to the meta-data
...
saveRDS(se.cca_cc, "~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/supplementary/se_cca_cc_object")

```


```{r, plot both dimensionality reductions and compare}

se.cca_cc <- readRDS("~/Desktop/se_cca_cc_object")
new.cluster.ids <- c("portal_cluster", "0", "central_cluster", "immune_cluster", "4", "cluster_5")

names(new.cluster.ids) <- levels(se.cca_cc)

se.cca_cc <- RenameIdents(se.cca_cc, new.cluster.ids)

DimPlot(se.cca, pt.size = 1 , label = T, cols = cluster.cols)

cluster.cols.cc = c("blue3","yellow3", "red3", "turquoise3","green4","darkorange1")

DimPlot(se.cca_cc, pt.size = 1 , label = T, cols = cluster.cols.cc)

ST.FeaturePlot(se.cca_cc, features = "seurat_clusters", indices = 3, cols = cluster.cols.cc)

ST.FeaturePlot(se.cca, features = "seurat_clusters", indices = 3, cols = cluster.cols)
...
```{r, export umap coordinates for entropy analysis}

#without cellcount norm

crd.1 <- as.data.frame(se.cca@reductions$umap@cell.embeddings)
crd.1$cluster <- se.cca@meta.data$seurat_clusters

write.table(crd.1, file = "~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev3/original_cluster_crd.tsv", sep = "\t", quote = F, row.names = rownames(crd.1), col.names = colnames(crd.1))
#with cellcount norm 

crd.2 <- as.data.frame(se.cca_c@reductions$umap@cell.embeddings)
crd.2$cluster <- se.cca_c@meta.data$seurat_clusters

write.table(crd.2, file = "~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev3/cellcountnorm_cluster_crd.tsv", sep = "\t", quote = F, row.names = rownames(crd.2), col.names = colnames(crd.2))

```


```{r, compare the cluster-annotations between datasets}


#extract the seurat clusters from the cell-count normalized data and append the cluster annotations to the original (non-cellcount normalized data)
cc_clusters <- se.cca.3.cc[["seurat_clusters"]]

se.cca$cc_seurat_clusters <- cc_clusters
#get new and old labels
convert_old <- c("0" = "cluster 0", "1" = "portal cluster","2" = "central cluster", "3" = "heme cluster", "4" = "immunity cluster", "5" = "cluster 5")
convert_new <- c("0" = "cluster 0", "1" = "portal cluster","2" = "central cluster", "3" = "heme cluster", "4" = "cluster 5", "5" = "immunity")


se.cca$label_old <- convert_old[as.character(se.cca$seurat_clusters)]
se.cca$label_new<- convert_new[as.character(se.cca_cc$seurat_clusters)]


tab <- table(se.cca@meta.data[, c("seurat_clusters", "cc_seurat_clusters")])
tab <- table(se.cca@meta.data[, c("label_old", "label_new")])

write.table(tab, "~/Desktop/table1.tsv", quote = F, sep = "\t")

tab

df <- data.frame("original_norm_clusters" = se.cca$seurat_clusters, "cellcount_norm_clusters" = se.cca$cc_seurat_clusters, "annotation_original" = se.cca$label_old, "annotation_cellcount" = se.cca$label_new)

write.table(df, "~/Desktop/clusters_LL.tsv", sep = "\t" , row.names = rownames(df), quote = F)

png(filename = "~/Desktop/PhDprojectFranziskaHildebrandt/Publication_Liver/revision/revision_plots/rev3/DimPlot_cc_norm.png", width = 4200, height = 3000, res = 300) #Units are pixels by default
print(DimPlot(se.cca_cc, pt.size = 1.2, cols = cluster.cols.cc) +
...
mclust::adjustedRandIndex(se.cca$label_old, se.cca$label_new)
